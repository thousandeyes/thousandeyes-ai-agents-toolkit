# ThousandEyes Synthetic Test Management Reference
...
## Scheduled Test Types and Required Fields

Use the narrowest valid scheduled test type for the target.

| Test Type | Best For | Required Fields |
| --- | --- | --- |
| `agent-to-server` | Network reachability, path, latency | `test_name`, `server`, `agent_ids`; often `protocol` |
| `agent-to-agent` | Inter-site connectivity | `test_name`, `agent_ids`, `target_agent_id`; often `protocol`, `port` |
| `http-server` | URL availability and response timing | `test_name`, `url`, `agent_ids` |
| `page-load` | Browser page performance | `test_name`, `url`, `agent_ids` |
| `dns-server` | DNS response time and availability | `test_name`, `domain`, `dns_servers`, `agent_ids` |
| `dns-trace` | Delegation-chain validation | `test_name`, `domain`, `agent_ids` |
| `dnssec` | DNSSEC validation | `test_name`, `domain`, `agent_ids` |
| `api` | Multi-step API workflows | `test_name`, `url`, `requests_config`, `agent_ids` |
| `web-transactions` | Scripted browser workflows | `test_name`, `url`, `transaction_script`, `agent_ids` |
| `ftp-server` | FTP service checks | `test_name`, `url`, `agent_ids` |
| `sip-server` | SIP infrastructure checks | `test_name`, `target_sip_credentials`, `agent_ids` |
| `voice` | Voice/SIP quality workflows | `test_name`, `target_sip_credentials`, `agent_ids` |
| `bgp` | Route monitoring by prefix | `test_name`, `prefix` |
...
## Instant-Test Selection Guide

Use instant tests when the user wants an immediate check before committing to a scheduled test:
...
If the user wants to keep the monitoring after validation, recommend converting the successful instant-test design into a scheduled test with the matching type.
...
Use `get_templates` when the user wants prebuilt SaaS monitoring, especially for products such as Microsoft 365, Salesforce, Slack, Zoom, Okta, or ServiceNow.
...
Prefer templates when they replace a complex multi-test manual setup.
...
For a new scheduled monitoring workflow:

1. Resolve the target type and agent scope.
2. If the user wants validation first, run the matching instant test.
3. Summarize the planned scheduled-test payload.
4. After confirmation, call `create_synthetic_test`.
5. Return the new test identity and recommended follow-up actions, such as alerting or wider agent coverage.
